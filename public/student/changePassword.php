<?php
	...
	if (isset($_POST['submit'])) {
		...
		if ($currentPassword!=""&&$confirmPassword!=""&&$confirmPassword==$newPassword) {
			$stmt =mysqli_query($conn, "SELECT password FROM user WHERE username='$matrixNo'");
			while($row=mysqli_fetch_array($stmt)){
				if ($currentPassword==$row['password']) {
					mysqli_query($conn, "UPDATE user SET password='$confirmPassword' WHERE username='$matrixNo'");
...
<head>
	...
	<title>Change Password | Student | UReCo</title>
