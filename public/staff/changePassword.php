<?php
	...
	if (isset($_POST['submit'])) {
		...
		if ($currentPassword!=""&&$confirmPassword!=""&&$confirmPassword==$newPassword) {
			$stmt =mysqli_query($conn, "SELECT password FROM user WHERE username='$staffID'");
			while($row=mysqli_fetch_array($stmt)){
				if ($currentPassword==$row['password']) {
					mysqli_query($conn, "UPDATE user SET password='$confirmPassword' WHERE username='$staffID'");
...
<head>
	...
	<title>Change Password | Staff | UReCo</title>
