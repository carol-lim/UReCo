<?php
	include("auth.php");
	include("../config.php");
	$maint_block = $_SESSION['block_id'];
?>
<!DOCTYPE html>
<html>
<head>
	<?php include("head.html");?>
	<title>Manage Helpdesk | Maintenance Team | UReCo</title>
		...
		<div class="d-flex flex-column " id="content">
			<nav aria-label="breadcrumb">
			  	<ol class="breadcrumb">
			    	...
			    	<li class="breadcrumb-item active" aria-current="page">Manage Helpdesk</li>
			  	</ol>
			</nav>			
			<div class="d-flex">
				<div class="p-2">
					<h3 id="heading1">Manage Helpdesk <?php echo " (". $_SESSION['college_name'] ." ". $_SESSION['block_name'].")"; ?></h3>
				</div>
			</div>
			...
			<div class="rounded p-4 bg-color">
				<div class="table-responsive">  
					<table id="helpdesk_current" class="table table-striped table-bordered dt-responsive wrap" style="width:100%">  
						...
						<tbody>  
						<?php  
						$query = mysqli_query($conn, "SELECT helpdesk.*, category.categoryName, subcategory.scName FROM helpdesk JOIN category ON helpdesk.category = category.categoryID JOIN subcategory ON helpdesk.subcategory = subcategory.scID JOIN student ON helpdesk.matrixNo = student.matrixNo JOIN block ON student.blockID = block.blockID WHERE student.blockID = '$maint_block' AND mtView IN(1,2) AND closeDate IS NULL"); 
